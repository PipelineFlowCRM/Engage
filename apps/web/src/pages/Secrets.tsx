...
export function Secrets() {
  ...
  const { data } = useQuery({
    queryKey: ['secrets'],
    queryFn: () => api.get<{ secrets: Secret[] }>('/secrets'),
  });
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      toast.success('SES secret saved — restart workers to pick up new credentials');
      setAccessKeyId('');
      setSecretAccessKey('');
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Encrypted credentials for sending. Live SES quota displays once configured.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {sesSecret ? (
          ...
          >
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label>Default from domain (optional)</Label>
                <Input
                  value={defaultFromDomain}
                  onChange={(e) => setDefaultFromDomain(e.target.value)}
                />
              </div>
            </div>
            <div className="space-y-2">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Live SES quota</CardTitle>
          <CardDescription>The worker polls every 60s once SES creds are saved.</CardDescription>
        </CardHeader>
        <CardContent>
          {!sesQuota ? (
            <p className="text-sm text-muted-foreground">Waiting for first quota poll.</p>
          ) : (
            <ul className="space-y-2 text-sm">
              <li className="flex justify-between">
                <span className="text-muted-foreground">Production access</span>
                {sesQuota.productionAccessEnabled ? (
              ...
              <li className="flex justify-between">
                ...
                <span className="tabular">
                  {sesQuota.sentLast24h.toLocaleString()} / {sesQuota.max24h.toLocaleString()}
                </span>
              </li>
              <li className="flex justify-between">
                <span className="text-muted-foreground">Max send rate</span>
                <span className="tabular">{sesQuota.maxSendRate}/sec</span>
              </li>
              <li className="flex justify-between text-xs text-muted-foreground">
                <span>Last poll</span>
                <span>{relativeTime(sesQuota.pollAt)}</span>
              </li>
            </ul>
          )}
        </CardContent>
