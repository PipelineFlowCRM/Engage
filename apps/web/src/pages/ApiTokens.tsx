...
export function ApiTokens() {
  ...
  const { data } = useQuery({ queryKey: ['api-tokens'], queryFn: () => api.get<{ tokens: Token[] }>('/api-tokens') });
  ...
  const create = useMutation({
    ...
    onSuccess: (res) => { setIssued(res.secret); setName(''); qc.invalidateQueries({ queryKey: ['api-tokens'] }); },
  ...
  return (
    <div>
      <PageHeader title="API tokens" description="Bearer tokens for /api/public/track and admin automation. Shown only once at creation — copy immediately." />
      <div className="grid grid-cols-1 gap-4 p-6 md:grid-cols-2">
        <Card>
          ...
          <CardContent>
            <form onSubmit={(e) => { e.preventDefault(); create.mutate(); }} className="space-y-3">
              ...
              <Button type="submit" variant="brand" disabled={create.isPending || !scopes.length}>
            ...
            {issued ? (
              <div className="mt-4 rounded-md border border-warning/40 bg-warning/5 p-3 text-xs">
                ...
                <Button size="sm" variant="outline" className="mt-2" onClick={() => { void navigator.clipboard.writeText(issued); toast.success('Copied'); }}>Copy</Button>
              </div>
            ) : null}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {!data?.tokens.length ? <p className="text-sm text-muted-foreground">No tokens.</p> : (
                  ...
                  <li key={t.id} className="flex items-center justify-between gap-3 py-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 text-xs text-muted-foreground">
                        {t.revokedAt ? `revoked ${relativeTime(t.revokedAt)}` : t.lastUsedAt ? `last used ${relativeTime(t.lastUsedAt)}` : 'never used'}
                      </div>
                    </div>
                    {!t.revokedAt ? (
